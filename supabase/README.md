...
`messages`, `notifications`, 승인 큐, 회원 상태, 게시물·댓글, 대화 읽음 상태는 `supabase_realtime` publication에 멱등 등록됩니다. 클라이언트 Realtime 구독에도 각 사용자의 RLS가 그대로 적용됩니다.
...
- `organization_memberships`: `id`, `user_id`, `organization_id`, `role`, `status`, `approved_from_application_id`, `approved_by`, `joined_at`, `ended_at`, `updated_at`
- `membership_applications`: `id`, `user_id`, `organization_id`, `requested_role`, `status`, `applicant_note`, `evidence_path`, `review_reason`, `reviewed_by`, `reviewed_at`, timestamps
...
submit_membership_application(p_organization_id uuid, p_requested_role app_role, p_applicant_note text = null) -> uuid
